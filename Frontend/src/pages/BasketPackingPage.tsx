...
import { fetchBasketPacking, scanBasketItem, completePacking } from '../api/client';
...
export const BasketPackingPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetchBasketPacking(basketId);
      setItems(res.items);
    } catch (err) {
      console.error(err);
  ...
  const handleSkuScan = async (e: React.FormEvent) => {
    ...
    if (targetItem) {
      try {
        await scanBasketItem(basketId, input);
        setItems(prev => {
          const idx = prev.findIndex(it => it.sku.toUpperCase() === input && it.packedQty < it.requiredQty);
          if (idx === -1) return prev;
          const updated = [...prev];
          updated[idx] = { ...updated[idx], packedQty: updated[idx].packedQty + 1 };
          return updated;
        });
        setSkuScan('');
      } catch (err: any) {
  ...
  const handleConfirmPack = async () => {
    ...
    setSubmitting(true);
    try {
      // For basket packing, we complete all orders in the basket
      const orderNumbers = Array.from(new Set(items.map(it => it.orderNumber)));
      for (const orderNum of orderNumbers) {
        await completePacking(orderNum as string);
      }
      navigate('/packing');
    } catch (err: any) {
  ...
  if (loading) return <div className="p-8 text-center text-xxs font-bold text-neutral-400">Loading Basket...</div>;
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-neutral-50/50 dark:bg-[#1c1c1c]/50 p-2 rounded-md border border-neutral-100 dark:border-neutral-900">
        <div className="text-left">
          ...
          <p className="text-[9px] text-neutral-400 font-bold mt-0.5">
            Multi-order basket verification
          </p>
        </div>
        <Button
          variant="secondary"
          ...
          <div className="bg-white dark:bg-[#232323] border border-neutral-100 dark:border-neutral-950 rounded-md overflow-hidden shadow-sm text-left">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-neutral-50 dark:divide-neutral-950/30">
                {items.map((it, idx) => (
                  <tr key={`${it.orderNumber}-${it.sku}-${idx}`} className={`${it.packedQty >= it.requiredQty ? 'opacity-25' : ''} transition-colors`}>
                    <td className="px-3 py-2 text-[9px] font-black text-neutral-500 uppercase">{it.orderNumber}</td>
                    <td className="px-3 py-2">
                      <div className="text-[10px] font-black text-black dark:text-white leading-none uppercase tracking-tighter">{it.sku}</div>
                    </td>
                    <td className="px-3 py-2 text-[9px] font-bold text-neutral-400 truncate max-w-[150px]">{it.itemName}</td>
        ...
        <div className="space-y-3">
          <div className="p-3 border border-neutral-100 dark:border-neutral-950 rounded-md bg-white dark:bg-[#232323] shadow-sm space-y-3">
            ...
            <Button
              ...
              loading={submitting}
            >
              {isComplete ? 'Generate Manifests' : `${totalRequired - totalPacked} Units Left`}
            </Button>
          </div>
        </div>
      </div>
